...
- Retention: keep recent runs by default (for example 30 days); provide a CLI
  cleanup command or a `CODEX_TOOLLOOP_RETENTION_DAYS`-style env to override,
  and delete per-run dirs plus their index entries.
- Size limits: cap per-run size (for example 100 MB) and per-file size for
  `events.jsonl`, `codex-events.jsonl`, and `tool-calls.jsonl`; when limits are
  exceeded, truncate with a clear marker or roll files to
  `*.jsonl.<n>.gz`/archived variants while keeping `meta.json` small.
- Index recovery: write `index.jsonl` entries atomically (write + rename) and
  validate on read; on corruption, rebuild by scanning `~/.codex-toolloop/runs/*`
  and rehydrating index entries from each run's `meta.json`.
...
- Add CLI tests that:
  ...
  - validate streaming output by capturing stdout/stderr and asserting that
    incremental messages arrive before completion
  - run `codex-toolloop session list` and `codex-toolloop session inspect <id>`
    using the temp HOME artifact store
  - confirm index entries exist after a run by reading `index.jsonl` and
    asserting metadata matches `meta.json`
